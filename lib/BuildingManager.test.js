...
describe("BuildingManager", () => {
  ...
  it("should show heros as available if one is built and tier 2 building is produced", () => {
    ...
    const strongholdData = Orc.buildings.find((b) => b.id === "stronghold");
    const stronghold = new Building(strongholdData);

    const units = [bladeMaster];
    const buildings = [altar, greatHall, stronghold];
  ...
  it("should not show heros as available if two are built and tier 3 building is not produced", () => {
    ...
    const taurenChieftainData = Orc.units.find(
      (u) => u.id === "taurenChieftain"
    );
  ...
  it("should show heros as available if two are built and tier 3 building is produced", () => {
    ...
    const fortressData = Orc.buildings.find((b) => b.id === "fortress");
    const fortress = new Building(fortressData);

    const units = [bladeMaster, shadowHunter];
    const buildings = [altar, greatHall, stronghold, fortress];
