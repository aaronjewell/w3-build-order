...
import itemData from "../data/orc/items"
...
describe("Action", () => {
  ...
  describe("buy", () => {
    it("should return a new buy action", () => {
      const start = 0
      const voodooLounge = new Building(
        buildingData.find(b => b.id === "voodooLounge"),
      )
      const healingSalveData = itemData.find(i => i.id === "healingSalve")

      const action = Action.buy(start, healingSalveData, voodooLounge)

      expect(action.type).toEqual("buy")
      expect(action.start).toEqual(start)
      expect(action.duration).toEqual(0)
      expect(action.meta.item).toEqual(healingSalveData)
      expect(action.meta.building).toEqual(voodooLounge)
    })
  })
})
